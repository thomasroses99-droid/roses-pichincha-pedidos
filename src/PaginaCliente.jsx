...
// ── Promo Hot Sale ──────────────────────────────────────────────────
const PROMO_SIMPLE = 12000;
const PROMO_DOBLE  = 14000;
function esHotSale() {
          ...
          return (
            <div key={item.cartId} style={{ background: "#FDFAF5", borderRadius: 12, padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "flex-start", gap: 10, boxShadow: "0 1px 4px #0001" }}>
              ...
              <div style={{ textAlign: "right" }}>
                {desc > 0 && <div style={{ fontSize: 11, color: "#bbb", textDecoration: "line-through" }}>{fmt(item.precio)}</div>}
                <div style={{ fontWeight: 700, color: G, fontSize: 14 }}>{fmt(pItem)}</div>
              </div>
        ...
        <div style={{ paddingTop: 8, marginBottom: 24 }}>
          ...
          {hayPromo && (
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 4 }}>
              <span style={{ fontSize: 12, color: G, fontWeight: 700 }}>{descuentoHoy ? "🎉 20% OFF de hoy aplicado" : promoActiva ? `🎉 Precio ${esHotSale() ? "Hot Sale" : "promo"} aplicado` : "🏷️ Descuento fin de semana Cheeseburger aplicado"}</span>
            </div>
          )}
  ...
  async function confirmarPedido({ nombre, telefono, dir, localidad, costoEnvio, localidadNombre, tipo: t, pago, notas, coords }) {
    ...
    carrito.forEach(item => {
      if (item.tipo === "burger") {
        ...
        lineas.push(`• 🍔 ${item.nombre} (${item.tamano}) — ${fmt(pItem)}${descFds > 0 ? ` 🏷️-${Math.round(descuentoFdsPct(pago) * 100)}%` : ""}`);
